    ...
    while (true)
    {
        ...
        while (true)
        {
            ...
            if (bytes_received > 0)
            {
                ...
                std::cout << "Client choice: " << message << "\n";
                ...
                else if (message == "3")
                {
                    ...
                    int message_bytes = recv(client_fd, message_buffer, sizeof(message_buffer) - 1, 0);

                    if (message_bytes > 0)
                    {
                        message_buffer[message_bytes] = '\0';
                        std::string user_message = message_buffer;
